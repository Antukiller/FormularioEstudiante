...
interface EstudianteDao {
    ...
    @SqlUpdate("INSERT INTO Estudiante (nia, curso, apellidos, nombre, email, fechaNacimiento, calificacion, repetidor, imagen, created_at, updated_at) VALUES (:nia, :curso, :apellidos, :nombre, :email, :fechaNacimiento, :calificacion, :repetidor, :imagen, :createdAt, :updatedAt)")
    @GetGeneratedKeys
    fun insert(@BindBean Estudiante: EstudianteEntity): Long

    @SqlUpdate("UPDATE Estudiante SET nia = :nia, curso = :curso, apellidos = :apellidos, nombre = :nombre, email = :email, fechaNacimiento = :fechaNacimiento, calificacion = :calificacion, repetidor = :repetidor, imagen = :imagen, updated_at = :updatedAt WHERE id = :id")
