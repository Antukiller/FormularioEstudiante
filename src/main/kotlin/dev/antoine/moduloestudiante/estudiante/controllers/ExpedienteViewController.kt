package dev.antoine.moduloestudiante.estudiante.controllers

import com.github.michaelbull.result.*
import dev.antoine.moduloestudiante.estudiante.errors.EstudianteError
import dev.antoine.moduloestudiante.estudiante.viewmodels.ModuloViewModel.TipoOperacion.EDITAR
import dev.antoine.moduloestudiante.estudiante.viewmodels.ModuloViewModel.TipoOperacion.NUEVO
...
class DetalleViewController : KoinComponent {
    ...
    val viewModel: ModuloViewModel by inject()
    ...
    @FXML
    private lateinit var textEstudianteNia: TextField

    @FXML
    private lateinit var textEstudianteCurso: TextField
    ...
    private fun initValues() {
        ...
        textEstudianteNia.text = viewModel.state.value.estudiante.nia
        textEstudianteCurso.text = viewModel.state.value.estudiante.curso
       ...
       // Dependiendo del modo
        validateForm().andThen {
            ...
            viewModel.updateDataEstudianteOperacion(
                nia = textEstudianteNia.text,
                curso = textEstudianteCurso.text,
    ...
    private fun limpiarFromulario() {
        ...
        textEstudianteNia.clear()
        textEstudianteCurso.clear()
    ...
    private fun validateForm(): Result<Unit, EstudianteError> {
        ...
        if (textEstudianteNia.text.isNullOrEmpty() || !textEstudianteNia.text.matches(Regex("^\\d{8}$"))) {
            return Err(EstudianteError.ValidationProblem("El NIA debe tener 8 dígitos numéricos"))
        }


        if (textEstudianteCurso.text.isNullOrEmpty() || (textEstudianteCurso.text != "1ºDAW" && textEstudianteCurso.text != "2ºDAW")) {
