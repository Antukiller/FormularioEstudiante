...
import dev.antoine.moduloestudiante.estudiante.viewmodels.ModuloViewModel.TipoOperacion
...
class FormularioViewController : KoinComponent {
    ...
    private fun initBindings() {
        ...
        textEstudianteNia.textProperty().bind(viewModel.state.map { it.estudiante.nia })
        textEstudianteCurso.textProperty().bind(viewModel.state.map { it.estudiante.curso })
    ...
    private fun initEventos() {
        ...
        botonEditar.setOnAction { onEditarAction() }
    ...
    private fun filterDataTable() {
        ...
        tableEstudiante.items =
            FXCollections.observableArrayList(viewModel.estudiantesFilteredList(comboTipo.value, textBuscador.text.trim()))
    }
    ...
    private fun onEditarAction() {
        ...
        viewModel.changeEstudianteOperacion(TipoOperacion.EDITAR)
        RoutesManager.initDetalle()
    }

    private fun onNuevoAction() {
        logger.debug { "onNuevoAction" }
        viewModel.changeEstudianteOperacion(TipoOperacion.NUEVO)
        RoutesManager.initDetalle()
    }
