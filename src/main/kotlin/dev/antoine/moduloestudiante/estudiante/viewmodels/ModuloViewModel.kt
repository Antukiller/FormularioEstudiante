package dev.antoine.moduloestudiante.estudiante.viewmodels
...
import dev.antoine.moduloestudiante.locale.round
...
class ModuloViewModel (
    ...
    fun estudiantesFilteredList(tipo: String, nombreCompleto: String): List<Estudiante> {
        ...
        return state.value.estudiantes
            .filter { estudiante ->
                when (tipo) {
                    ...
                    TipoFiltro.PRIMERO_DAW.value -> estudiante.curso == "1ºDAW"
                    TipoFiltro.SEGUNDO_DAW.value -> estudiante.curso == "2ºDAW"
                    else -> true
                }
            }.filter { estudiante ->
    ...
    enum class TipoFiltro(val value: String) {
        TODOS("Todos/as"), SI("Rep: Sí"), NO("Rep: No"), PRIMERO_DAW("1ºDAW"), SEGUNDO_DAW("2ºDAW")
    }
